...
bool tbuf_current_at_start(tbuf tb);        // Checks if current is at start
bool tbuf_current_at_end(tbuf tb);          // Checks if current is at end
void tbuf_forward(tbuf tb);                 // Moves the current to the right (end)
void tbuf_backward(tbuf tb);                // Moves the current to the left (start)
void tbuf_delete_current(tbuf tb);          // Removes the current node from list
